...
(() => {
  ...
  function manualPanel(type){const duration=Math.max(.001,num($("manualDuration")?.value,state.manualDuration||60));state.manualDuration=duration;const view=state[type].view||"profile-right", side=view==="profile-left"?"left":"right", box=$(type==="shoulder"?"preparedStudy":type==="elbow"?"elbowStudy":"wristStudy"), d=state.manual[type][side]||{};let title=type==="shoulder"?"hombro":type==="elbow"?"codo":"muñeca",th=type==="shoulder"?80:type==="elbow"?60:(state.wrist.threshold||60);let html='<div class="manual-entry"><h3>Entrada manual · '+title+'</h3><p class="analysis-help">Introduzca ángulo y tiempo de exposición. El porcentaje se calcula automáticamente respecto al tiempo total de análisis ('+fmt(duration,3)+' s).</p><div class="analysis-grid">'+mf(type+'View','Vista',view)+'</div>';
    ...
    html+='<button type="button" class="nav-primary" id="manualCalc">Calcular '+title+'</button><div id="manualResult" class="shoulder-result"><div class="placeholder">Introduzca los datos y calcule.</div></div></div>';box.innerHTML=html;const v=$(type+'View');v.outerHTML='<label>Vista<select id="'+type+'View"><option value="profile-right">Perfil derecho</option><option value="profile-left">Perfil izquierdo</option><option value="frontal">Frontal</option></select></label>';;$(type+'View').value=view;$(type+'View').onchange=()=>{captureManual(type);state[type].view=$(type+'View').value;manualPanel(type);};$("manualCalc").onclick=()=>{state[type].view=$(type+'View').value;captureManual(type);$("manualResult").innerHTML=manualCalculate(type);};}
  function captureManual(type){const view=state[type].view||"profile-right";if(view==="frontal"){["right","left"].forEach(s=>{if(type==="shoulder")state.manual[type][s]={angle:mv('mS'+s+'A'),time:mv('mS'+s+'T')};if(type==="elbow")state.manual[type][s]={pronAngle:mv('mE'+s+'PA'),pronTime:mv('mE'+s+'PT'),supAngle:mv('mE'+s+'SA'),supTime:mv('mE'+s+'ST')};if(type==="wrist")state.manual[type][s]={radialAngle:mv('mW'+s+'RA'),radialTime:mv('mW'+s+'RT'),cubitalAngle:mv('mW'+s+'CA'),cubitalTime:mv('mW'+s+'CT')};});}else{const s=view==="profile-left"?"left":"right",p=type[0].toUpperCase()+type.slice(1)+s;state.manual[type][s]={flexAngle:mv('m'+p+'FA'),flexTime:mv('m'+p+'FT'),extAngle:mv('m'+p+'EA'),extTime:mv('m'+p+'ET')};}}
  ...
  function renderShoulder() {
    if(state.inputMode==="manual"){manualPanel("shoulder");return;}
  ...
  function renderElbow() {
    if(state.inputMode==="manual"){manualPanel("elbow");return;}
  ...
  function renderWrist() {
    if(state.inputMode==="manual"){manualPanel("wrist");return;}
