...
const values=()=>{const o={savedAt:new Date().toISOString(),values:{},kinovea:{...kinoveaState,videoUrl:""},recovery:{...recoveryState,pauses:(recoveryState.pauses||[]).map(p=>({...p}))}};fields.forEach(f=>{if(f.type==="checkbox")o.values[f.name]=f.checked;else if(f.type==="radio")o.values[f.name]=f.checked?f.value:"";else o.values[f.name]=f.value});if(typeof simulationState!=="undefined"&&simulationState.initialised){o.simulation={version:1,baseline:simClone(simulationState.baseline||{}),current:simClone(simulationState.current||simReadState())};}return o};
