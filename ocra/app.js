...
function renderFinalBreakdown(prefix,metrics){
 const forceRows=[["Borg 3–4",metrics.force34],["Borg 5–7",metrics.force57],["Borg 8–10",metrics.force810]];
 const forceHtml=forceRows.map(([label,value])=>"<tr><td>"+label+"</td><td>"+fmt(value,2)+"</td></tr>").join("");
 const compA=[...form.querySelectorAll("[data-comp-a]")].filter(x=>x.checked);
 const compRows=compA.length?compA.map(x=>"<tr><td>"+x.parentElement.textContent.trim()+"</td><td>"+fmt(parseFloat(x.dataset.compA)||0,2)+"</td></tr>").join(""):"<tr><td colspan=\"2\">Ningún factor A seleccionado</td></tr>";
 const bValue=parseFloat(form.elements.complementarioB.value)||0;
 const bText=form.elements.complementarioB.selectedOptions?.[0]?.textContent||"Ninguna";
 const postureRows="<tr><td>Hombro</td><td>—</td><td>No puntuado todavía</td></tr><tr><td>Codo</td><td>—</td><td>No puntuado todavía</td></tr><tr><td>Muñeca</td><td>—</td><td>No puntuado todavía</td></tr><tr><td>Mano</td><td>—</td><td>No puntuado todavía</td></tr><tr><td>Estereotipia</td><td>"+fmt(metrics.stereo,2)+"</td><td>Incluida en el cálculo actual</td></tr>";
 const el=document.getElementById("finalBreakdown"+prefix);if(!el)return;
 el.innerHTML="<div class=\"breakdown-section\"><strong>Frecuencia</strong><table class=\"compact-table\"><tbody><tr><td>Acciones/min</td><td>"+fmt(metrics.actionsMin,2)+"</td></tr><tr><td>Puntuación frecuencia</td><td>"+fmt(metrics.frequency,2)+"</td></tr></tbody></table></div><div class=\"breakdown-section\"><strong>Factor fuerza · escala de Borg</strong><table class=\"compact-table\"><tbody>"+forceHtml+"<tr><th>Total factor fuerza</th><th>"+fmt(metrics.force,2)+"</th></tr></tbody></table></div><div class=\"breakdown-section\"><strong>Postura y estereotipia</strong><table class=\"compact-table\"><thead><tr><th>Componente</th><th>Puntuación</th><th>Estado</th></tr></thead><tbody>"+postureRows+"<tr><th>Total utilizado actualmente</th><th>"+fmt(metrics.stereo,2)+"</th><th>Postura + estereotipia</th></tr></tbody></table><div class=\"notice\">Las pantallas de postura muestran los datos observados, pero la puntuación OCRA de hombro, codo, muñeca y mano todavía no está integrada en la fórmula final. No se inventa una puntuación y el cálculo vigente utiliza únicamente la estereotipia.</div></div><div class=\"breakdown-section\"><strong>Factores complementarios</strong><table class=\"compact-table\"><thead><tr><th>Factor</th><th>Puntuación</th></tr></thead><tbody>"+compRows+"<tr><td>"+bText+"</td><td>"+fmt(bValue,2)+"</td></tr><tr><th>Total complementarios</th><th>"+fmt(metrics.comp,2)+"</th></tr></tbody></table></div><div class=\"breakdown-section\"><strong>Multiplicadores</strong><table class=\"compact-table\"><tbody><tr><td>Recuperación</td><td>× "+fmt(metrics.recovery,3)+"</td></tr><tr><td>Duración</td><td>× "+fmt(metrics.duration,3)+"</td></tr></tbody></table></div>";
}
function calculate(){
 ...
 const actionCycle=obs||cycle,dxA=n("dxAcciones"),ixA=n("ixAcciones"),dxMin=actionCycle>0?dxA*60/actionCycle:0,ixMin=actionCycle>0?ixA*60/actionCycle:0,dxF=freq(dxMin,form.elements.dxInterrupciones.value==="si"),ixF=freq(ixMin,form.elements.ixInterrupciones.value==="si"),dxForce34=actionCycle>0?lookup(force34,n("dxFuerza34")/actionCycle):0,dxForce57=actionCycle>0?lookup(force57,n("dxFuerza57")/actionCycle):0,dxForce810=actionCycle>0?lookup(force810,n("dxFuerza810")/actionCycle):0,ixForce34=actionCycle>0?lookup(force34,n("ixFuerza34")/actionCycle):0,ixForce57=actionCycle>0?lookup(force57,n("ixFuerza57")/actionCycle):0,ixForce810=actionCycle>0?lookup(force810,n("ixFuerza810")/actionCycle):0,dxForce=dxForce34+dxForce57+dxForce810,ixForce=ixForce34+ixForce57+ixForce810,dxS=stereo("dx"),ixS=stereo("ix"),a=[...form.querySelectorAll("[data-comp-a]")].filter(x=>x.checked).reduce((s,x)=>s+(parseFloat(x.dataset.compA)||0),0),b=parseFloat(form.elements.complementarioB.value)||0,comp=a+b;
 ...
 renderFinalBreakdown("Dx",{actionsMin:dxMin,frequency:dxF,force34:dxForce34,force57:dxForce57,force810:dxForce810,force:dxForce,stereo:dxS,comp:comp,recovery:rm??1,duration:md});
 ...
 renderFinalBreakdown("Ix",{actionsMin:ixMin,frequency:ixF,force34:ixForce34,force57:ixForce57,force810:ixForce810,force:ixForce,stereo:ixS,comp:comp,recovery:rm??1,duration:md});
}
