...
function updateForceModeUI(){const mode=document.getElementById("fuerzaModo")?.value||"segundos";document.querySelectorAll(".fuerza-unidad").forEach(el=>el.textContent=mode==="porcentaje"?"% del tiempo de ciclo":"segundos/ciclo");document.querySelectorAll('input[name^="dxFuerza"],input[name^="ixFuerza"]').forEach(el=>{el.step=mode==="porcentaje"?"0.1":"0.1";el.max=mode==="porcentaje"?"100":"";el.placeholder=mode==="porcentaje"?"%":"s"});}
